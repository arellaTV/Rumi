### Imports
Multiple imports from the same package should be declared in alphabetical order.

### Asynchronous
Default to a promise when performing an async action.

* Resolve the promises by using a ternary operator, such as err ? reject(err) : resolve(data)
